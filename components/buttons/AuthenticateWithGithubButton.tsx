...
export default function AuthenticateWithGithubButton(props: { passErrorMessage: (errorMessage: string) => void; }) {
    ...
    async function handleLoginOrSignupWithGithub(event: any) : Promise<void> {
        ...
        const authenticationResponse = await authenticationService.loginOrSignupWithGithub()
    ...
    return(
        <button aria-label="Continue with github" onClick={handleLoginOrSignupWithGithub} role="button" className="focus:outline-none  focus:ring-2 focus:ring-offset-1 focus:ring-gray-700 py-3.5 px-4 border rounded-lg border-gray-700 flex items-center w-full mt-4">
